...
class Classifier(Network):
    ...
    def forward(self, X):
        ...
        X_bias = np.concatenate((X, np.ones((batch_size, 1))), axis=1)
        z = None
        ...
        s = X_bias.dot(self.W)  # Linear transformation (affine step)
        z = self.sigmoid(s)     # Activation
        self.cache = (X_bias, s, z) # Store X_bias (with ones), linear output s, and activated output z
    ...
    def backward(self, dout):
        ...
        X_bias, s, z = self.cache # z is sigmoid(s)
        # Gradient of loss w.r.t. sigmoid output z is dout (dL/dz)
        # Gradient of sigmoid output z w.r.t. its input s is dz/ds = z * (1 - z)
        # Gradient of loss w.r.t. linear output s is dL/ds = (dL/dz) * (dz/ds)
        dL_ds = dout * z * (1 - z)

        # Gradient of linear output s w.r.t weights W is ds/dW = X_bias.T
        # Gradient of loss w.r.t weights W is dL/dW = (dL/ds) * (ds/dW)
        # Using matrix calculus rules for shapes: X_bias (N, D+1), W (D+1, 1), s (N, 1)
        # dL_ds (N, 1)
        dW = X_bias.T.dot(dL_ds) # ((D+1), N) x (N, 1) -> (D+1, 1)
