...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # Reshape from (N, C, H, W) to (N*H*W, C) to use regular batchnorm
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # Reshape from (N, C, H, W) to (N*H*W, C) to use regular batchnorm backward
    dout_reshaped = dout.transpose(0, 2, 3, 1).reshape(N * H * W, C)
    
    # Apply regular batch normalization backward
