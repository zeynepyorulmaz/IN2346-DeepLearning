...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        # LeakyRelu: f(x) = max(slope * x, x) = x if x >= 0, slope * x if x < 0
    ...
    def backward(self, dout, cache):
        ...
        x = cache
        # Derivative: df/dx = 1 if x >= 0, slope if x < 0
...
class Tanh:
    ...
    def forward(self, x):
        ...
        # Tanh: f(x) = (e^x - e^(-x)) / (e^x + e^(-x)) = tanh(x)
        outputs = np.tanh(x)
        cache = outputs  # Store the output for efficient backward computation
    ...
    def backward(self, dout, cache):
        ...
        # Derivative: df/dx = 1 - tanh^2(x) = 1 - outputs^2
        outputs = cache
        dx = dout * (1 - outputs**2)
